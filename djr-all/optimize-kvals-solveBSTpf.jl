...
## Reload the PSM to have fresh data - don't need if running from the top
fname = "Feeder_Data/$(substation_name)/Python_Model/$(substation_name)_Model.pkl"
pkl_file = pyopen(fname, "rb")
psm = pickle.load(pkl_file)         # true model with AMI
...
# gens are 0)
write_netload_onetime(psm, nodes, worst_V_idx[1], Sload_worst[1,:], ph_col)


## Iterative simulation via function #####################################################
function simulate_BST_VVC(psm, V0_ref, t_ind, ph_col, kvals, nodes, gen_idx_in_209)
    # find initial voltage
    V_tmp = solve_pf(psm, V0_ref, t_ind, linear_solver)
    ...
    while iter < iter_max && V_diff > thresh
        ...
        for (ii,node) in enumerate(psm.Nodes[nodes])
            load_ind = node.loads[1] + 1
            sload = psm.Loads[load_ind].Sload
            sload[t_ind, ph_col] -= Q_out[ii]*1im         # first attempt
            # sload[t_ind, ph_col] += Q_out[ii]*1im           # change the sign of Q?
            # sload[t_ind, ph_col] = real(sload[t_ind, ph_col]) - Q_out[ii]*1im     # get rid of load and just use supplied Q?
            psm.Loads[load_ind].Sload = sload
        end
        # re-solve psm for voltage
        V_tmp = solve_pf(psm, V0_ref, t_ind, linear_solver)
        V_out .= abs.(V_tmp[ph_col,:])
    ...
    return Q_track, V_track
end
# artificially lower k
val, idx = findmin(kvals)
println("Max kval= $val")
kvals_copy = copy(kvals)
kvals_copy[idx] = -1
# solve
Q_track, V_track = simulate_BST_VVC(psm, V0_ref, worst_V_idx[1], ph_col, kvals_copy, nodes, gen_idx_in_209)
...
## Plot histograms of voltages before and after
Vinit = V_track[:,1]
Vfinal = V_track[:,end]
vmin = min(minimum(Vinit),minimum(Vfinal))
vmax = max(maximum(Vinit),maximum(Vfinal))
fig, axs = plt.subplots(2, 1, figsize=(6, 6)) 
axs[1].hist(Vinit, bins=10, alpha=0.7, label="Before VVC", color="blue")
axs[1].set_title("Dist. of Voltages with no VVC")
axs[1].set_xlabel("Voltage (pu)")
axs[1].set_ylabel("Frequency")
axs[1].legend()
axs[1].set_xlim(vmin, vmax)
axs[2].hist(Vfinal, bins=10, alpha=0.7, label="VVC", color="orange")
...
## Questions I have: 
# why is it not oscillating? In some cases, Q out decreases for a few in a row... 
# why is that one value so problematic? 
# why is the stability condition not actually stable? 